...
class CameraDyTrans : public rclcpp::Node
{
	public:
		CameraDyTrans() : Node("camera_dy_tf_broadcaster")
		{
			timer_ = create_wall_timer(std::chrono::milliseconds(10), std::bind(&CameraDyTrans::publishDynamicTransform, this));
...
int main(int argc, char **argv)
{
	...
	rclcpp::spin(std::make_shared<CameraDyTrans>());
	rclcpp::shutdown();
	return 0;
}