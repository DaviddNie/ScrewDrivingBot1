...
private:
    ...
    void setupHybridPlannerClient() {
        ...
        if (!hp_action_client_->wait_for_action_server(std::chrono::seconds(10))) {
            RCLCPP_ERROR(this->get_logger(), "Hybrid planning action server is not available.");
        }
    }

    void sendHybridPlanningGoal(const moveit_msgs::msg::MotionPlanRequest& motion_request) {
        auto goal = moveit_msgs::action::HybridPlanner::Goal();
        goal.planning_group = "ur_manipulator";  // Adjust to your robot's planning group
        moveit_msgs::msg::MotionSequenceItem sequence_item;
        sequence_item.req = motion_request;
        sequence_item.blend_radius = 0.0;  // Set blend radius
        goal.motion_sequence.items.push_back(sequence_item);

        auto send_goal_options = rclcpp_action::Client<moveit_msgs::action::HybridPlanner>::SendGoalOptions();
        send_goal_options.result_callback = [](const rclcpp_action::ClientGoalHandle<moveit_msgs::action::HybridPlanner>::WrappedResult& result) {
            if (result.code == rclcpp_action::ResultCode::SUCCEEDED) {
                RCLCPP_INFO(rclcpp::get_logger("ArmMovement"), "Hybrid planning goal succeeded.");
            } else {
                RCLCPP_ERROR(rclcpp::get_logger("ArmMovement"), "Hybrid planning goal failed.");
            }
        };

        send_goal_options.feedback_callback = [](const rclcpp_action::ClientGoalHandle<moveit_msgs::action::HybridPlanner>::SharedPtr&,
                                                const std::shared_ptr<const moveit_msgs::action::HybridPlanner::Feedback>& feedback) {
            RCLCPP_INFO_STREAM(rclcpp::get_logger("ArmMovement"), "Feedback received: " << feedback->feedback);
        };

        RCLCPP_INFO(this->get_logger(), "Sending hybrid planning goal...");
        hp_action_client_->async_send_goal(goal, send_goal_options);
    }
